...
export function activate(context: vscode.ExtensionContext) {
	...
	const registerFunction = (withComment: boolean) => {
		if (vscode.window.activeTextEditor) {
			...
			vscode.window.showTextDocument(viewUri, { preview: false });
		}
	};
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-ssi-viewer.load', () => { registerFunction(false); }),
		vscode.commands.registerCommand('vscode-ssi-viewer.load-with-comment', () => { registerFunction(true); })
	);
}
